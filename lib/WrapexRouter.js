...
WrapexRouter.prototype.post = function post(route, handlers, options) {
  return wrap(this, POST, route, handlers, options);
};

WrapexRouter.prototype.put = function put(route, handlers, options) {
  return wrap(this, PUT, route, handlers, options);
};

WrapexRouter.prototype.get = function get(route, handlers, options) {
  return wrap(this, GET, route, handlers, options);
};

WrapexRouter.prototype.delete = function deleteW(route, handlers, options) {
  return wrap(this, DELETE, route, handlers, options);
};

WrapexRouter.prototype.use = function use(route, handlers, options) {
...
function getHandlers({
  ...
  const allHandlers = optionalsMiddlewares.reduce(
    (arr, { option, handler, active }) => {
      const couldActivate = options[option] !== false;
      if (couldActivate && (active || !!options[option])) {
        arr.push(handler);
      }
      return arr;
    },
...
function wrap(wrapper, action, route, handlers, options) {
  ...
  router[action](
    route,
    ...getHandlers({ handlers, options, optionalsMiddlewares, onError }),
  );
  return wrapper;
}
